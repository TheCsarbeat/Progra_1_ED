...
void ThreadPrimerInspector::__init__(QMutex *mutex1,QMutex *mutex2, Horno * horno, Inspectores * inspectores,Inspector * inspector, Empacadora * empacadora, QCheckBox * checkOnOff){
...
void ThreadPrimerInspector::run(){
    ...
    while(running){
        ...
        while(paused){
            inspector->lbTitle->setText(inspector->nombre+" Waiting...");
        ...
        if(inspector->capacidad > (bandaSiguiente->capacidad - bandaSiguiente->cantNow)){
            ...
            inspector->lbTitle->setText(inspector->nombre+" Cargando...");
            inspector->actualGalletas++;
            bandaActual->cantNow--;
            inspectores->imprimir();
            msleep(500);
        }else if(inspector->capacidad <= inspector->actualGalletas){
            inspector->lbTitle->setText(inspector->nombre+" Descartando..");
